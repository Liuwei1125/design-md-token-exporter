import { isNeutralColor, normalizeColor, saturationScore } from './color-normalization';
import type { ColorElementSample, ColorEvidence, ColorEvidenceProperty, ObservedColorToken } from './types';

const COLOR_PROPERTIES: ColorEvidenceProperty[] = [
  'color',
  'backgroundColor',
  'borderTopColor',
  'borderRightColor',
...
export function extractColorTokens(samples: ColorElementSample[]): ObservedColorToken[] {
  const grouped = new Map<string, ColorEvidence[]>();
...
export function hasActionIntent(value: string | null | undefined): boolean {
...
export function extractSalientGradientColor(value: string | null | undefined): string | undefined {
  ...
  const candidates = [
    ...
    .map((candidate) => normalizeColor(candidate))
    .filter((candidate): candidate is string => Boolean(candidate));

  return candidates.find((candidate) => !isNeutralColor(candidate)) ?? candidates[0];
}

function extractEvidence(sample: ColorElementSample, property: ColorEvidenceProperty): ColorEvidence | undefined {
  const originalValue =
    property === 'backgroundColor'
      ? extractSalientGradientColor(sample.styles.backgroundImage) ?? sample.styles.backgroundColor
      : sample.styles[property as keyof typeof sample.styles];
  const normalized = normalizeColor(originalValue);
...
function scoreEvidence(
  sample: ColorElementSample,
  property: ColorEvidenceProperty,
  value: string,
  flags: { isAction: boolean; active: boolean; interactive: boolean; gradient: boolean },
  ...
  if (isNeutralColor(value) && (property === 'backgroundColor' || property === 'color')) {
    score += 3;
  }
...
function isInteractive(sample: ColorElementSample, context: string): boolean {
  const tag = sample.tagName.toLowerCase();
  return (
...
function isActiveState(sample: ColorElementSample, context: string): boolean {
  return Boolean(
    sample.ariaSelected ||
      sample.ariaCurrent ||
...
function sampleContext(sample: ColorElementSample): string {
  return [sample.tagName, sample.id, sample.className, sample.role, sample.text, sample.ariaCurrent].filter(Boolean).join(' ');
}

function collapseBorderProperty(property: ColorEvidenceProperty): ColorEvidenceProperty {
  return property.startsWith('border') ? 'borderColor' : property;
}

function usageForProperty(property: ColorEvidenceProperty): string {
  if (property === 'color') return 'text';
  if (property === 'backgroundColor') return 'background';
  if (property === 'outlineColor') return 'outline';
  if (property === 'textDecorationColor') return 'decoration';
  return 'border';
}

function saturationBonus(value: string, evidence: ColorEvidence[]): number {
  if (isNeutralColor(value)) return 0;
  const hasActionEvidence = evidence.some((item) => item.isActionIntent || item.isInteractive || item.isActiveState);
  return saturationScore(value) * (hasActionEvidence ? 36 : 14);
}

function warningsForToken(value: string, evidence: ColorEvidence[]): string[] {
  const warnings: string[] = [];
  if (!value.startsWith('#')) warnings.push('Modern or unresolved color format preserved without numeric color analysis.');
