import type { ClusterToken, LayoutElementSample, RadiusScaleName, RadiusTokens } from './types';

const RADIUS_PROPERTIES = ['borderTopLeftRadius', 'borderTopRightRadius', 'borderBottomRightRadius', 'borderBottomLeftRadius'] as const;

export function extractRadiusTokens(samples: LayoutElementSample[]): RadiusTokens {
...
function clusterRadiusValues(samples: LayoutElementSample[]): {
  values: ClusterToken<number>[];
  fullValues: Set<number>;
} {
  const grouped = new Map<number, { usage: Set<string>; evidence: string[]; frequency: number; fullFrequency: number; regularFrequency: number }>();
...
function inferRadiusScale(
  values: ClusterToken<number>[],
  fullValues: Set<number>,
): Partial<Record<RadiusScaleName, ClusterToken<number>>> {
...
function parseRadiusValue(value: string, sample: LayoutElementSample): number | undefined {
  const normalized = value.trim().toLowerCase();
  if (!normalized || normalized === '0px' || normalized === 'none') return undefined;
...
function isFullRadius(value: number, sample: LayoutElementSample): boolean {
