import type { ClusterToken, LayoutElementSample, SpacingScaleName, SpacingTokens } from './types';
...
export function extractSpacingTokens(samples: LayoutElementSample[]): SpacingTokens {
...
function clusterSpacingValues(samples: LayoutElementSample[]): ClusterToken<number>[] {
  const grouped = new Map<number, { usage: Set<string>; evidence: string[]; frequency: number }>();
  ...
  for (const sample of samples) {
    for (const property of SPACING_PROPERTIES) {
      ...
      const existing = grouped.get(value) ?? { usage: new Set<string>(), evidence: [], frequency: 0 };
      existing.usage.add(property);
      existing.frequency += 1;
      if (existing.evidence.length < 5) existing.evidence.push(`${describeSample(sample)} ${property}`);
...
function inferBaseUnit(values: ClusterToken<number>[]): 4 | 8 | 10 {
  const total = values.reduce((sum, token) => sum + token.frequency, 0);
  const scoreFor = (candidate: 4 | 8 | 10) =>
    values.reduce((sum, token) => {
  ...
  if (total > 0 && score10 / total >= 0.6 && score10 > score8) return 10;
  if (total > 0 && score8 / total >= 0.6) return 8;
  return 4;
}

function inferSpacingScale(values: ClusterToken<number>[]): Partial<Record<SpacingScaleName, ClusterToken<number>>> {
  const sorted = values.filter((token) => token.value > 0).sort((left, right) => left.value - right.value);
  const scale: Partial<Record<SpacingScaleName, ClusterToken<number>>> = {};
...
function nearest(values: ClusterToken<number>[], target: number): ClusterToken<number> | undefined {
  return values
...
function parseSpacingValue(value: string): number | undefined {
  ...
  if (!normalized || normalized === 'auto' || normalized === 'normal') return undefined;
